...
import { killAndWait, spawn, spawnLog } from "../../../lib/api-helper/misc/child_process";
...
describe("child_process", () => {
    ...
    describe("killAndWait", () => {
        ...
        it("should fall through to SIGKILL", async function t(this: Mocha.Context): Promise<void> {
            ...
            const script =
                "process.on('SIGTERM', function() { return; }); let t = setTimeout(function() { process.exit(7) }, 7000)";
            let closed = false;
            let exited = false;
            const cp = spawn("node", ["-e", script]);
            cp.on("exit", () => {
