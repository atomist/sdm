...
import {
    ...
    JavaPackageRegExp,
    MavenGroupIdRegExp,
} from "../../../../lib/pack/jvm/java/javaPatterns";
...
describe("javaPatterns", () => {
    describe("JavaIdentifierRegExp", () => {
        ...
        it("should reject invalid identifiers", () => {
            const identifiers = ["1x", "com-1", "com.foo-1.bar", "c%", "com.f"];
            identifiers.forEach(b => assert(!JavaIdentifierRegExp.pattern.test(b)));
        });
    });
    ...
    describe("MavenGroupIdRegExp", () => {
        it("should match valid ids", () => {
            const ids = ["atomist", "atomist-seeds", "com.foo.bar", "test_1"];
            ids.forEach(b => assert(MavenGroupIdRegExp.pattern.test(b)));
        });

        it("should reject invalid ids", () => {
            const ids = ["&345", "'66", "c%"];
            ids.forEach(b => assert(!MavenGroupIdRegExp.pattern.test(b), "Should have failed on " + b));
        });
    });
});
