...
export async function executeGoal(rules: { projectLoader: ProjectLoader, goalExecutionListeners: GoalExecutionListener[] },
    ...
    try {
        ...
        const result = handleGitRefErrors({ code: 1, ...(err.result || {}) });
        await notifyGoalExecutionListeners({
            ...inProcessGoalEvent,
...
function handleGitRefErrors(result: ExecuteGoalResult & any): ExecuteGoalResult {
    if (!!result?.error?.cause?.stderr) {
        const error = result.error.cause.stderr;
        if (/Remote branch .* not found/.test(error)) {
            result.state === SdmGoalState.canceled;
            result.phase = "branch not found";
        } else if (/reference is not a tree/.test(error)) {
            result.state === SdmGoalState.canceled;
